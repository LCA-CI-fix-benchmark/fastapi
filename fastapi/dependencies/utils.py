...
def get_dependant(
    *,
    ...
    if isinstance(depends.dependency, BaseModel):  # Check if dependency is a Pydantic model
        for query_param in dependant.query_params:
            query_param_field = depends.dependency.__fields__.get(query_param.name)
