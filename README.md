...
<p align="center">
<a href="https://github.com/tiangolo/fastapi/actions?query=workflow%3ATest+event%3Apush+branch%3* Check that it has a required attribute `price` that must be of type `float`.
* Check that it has an optional attribute `is_offer` of type `bool`, if present.
* Ensure that these validations apply to deeply nested JSON objects.
* Implement automatic conversion to and from JSON.ter" target="_blank">
